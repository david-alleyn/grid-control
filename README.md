...
- Developed in Python 3 using QT6 for the user interface
...
Grid Control automatically saves all settings when the application is closed ("x" in the top right corner). The settings are stored in the registry at ~/.config/GridControl/App.conf
...
- CachyOS: /dev/ttyACM0 is accessed by adding your user account or service account to the `uucp` group and restarting your computer.
- Other distros: Look it up. Possibly: `dialout`
...
- For editing the User Interface (`mainwindow.ui`), install QT Designer as follows:
  - Install latest qt6-base and qt-creator from your package manager.
